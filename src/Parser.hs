...
source >$> sink = Parser $ \i s ->
  case unParser source i s of
    Success is ss rs  -> case unParser sink rs ss of
...
updateState f parser = Parser $ \i s ->
  case unParser parser i s of
    Success i' s' r' -> Success i' (f s') r'
    failure@(Failure _ _ _) -> failure
...
sequenceP parsers = Parser $ \i s -> fromResult i s $ runParsers parsers i s
  where runParsers parsers' i s = case parsers' of
          [] -> Success i s []
          (p:ps) -> case unParser p i s of
            Success i' s' r -> Success i' s' (r:) <*> runParsers ps i' s'
            Failure i' s' errMsg -> Failure i' s' errMsg
        fromResult i s success@(Success _ _ _) = success
...
parseFail parser = Parser $ \i s ->
  case unParser parser i s of
    Success i' s' result' -> Failure i s
      $ ErrorOne "Failing parser succeeded instead."
...
parseOptional f = Parser $ \i s ->
  case i of
    [] -> Failure [] s $ ErrorOne
           ...
           <> "' but EOF found."
    (x:xs) -> case f x of
      Nothing -> Failure (x:xs) s $ ErrorOne
              $ "Attempting to parse'" <> show x <> "', but predicate wasn't satisfied."
      Just r -> Success xs s r

-- Similar to parseOne, but using ranges.
parseRange :: (Eq i, Show i, Ord i) => i -> i -> Parser i s i
parseRange lower upper = Parser $ \i s ->
  case i of
    [] -> Failure [] s $ ErrorOne
           $ "Attempted to parse '"
           <> show i
...
parseEof = Parser $ \i s ->
  case i of
    [] -> Success i s ()
    (x:xs) -> Failure i s $ ErrorOne
              $ "Attempted to parse EOF - elements still exist: "
              <> show i <> "."
...
parseOr parser1 parser2 = Parser $ \i s ->
  case unParser parser1 i s of
    success@(Success _ _ _) -> success
    Failure _ _ errMsg ->
      case unParser parser2 i s of
        success'@(Success _ _ _) -> success'
        Failure _ _ errMsg' ->
          Failure i s $ ErrorMore "Could not match alternatives." [errMsg, errMsg']
...
parseAp parserF parser = Parser $ \i s ->
...
parsePure result = Parser $ \i s ->
  Success i s result

-- Return a parser that consumes nothing and always fails.
parseEmpty :: Parser i s r
parseEmpty = Parser $ \i s ->
  Failure i s $ ErrorOne "Empty parser."
